...
const EditAdminAssistent = () => {
  ...
  const { isLoading, fetchElement, EditAdminAss } = useStore();
  useEffect(() => {
    const fetch = async () => {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      const response = await EditAdminAss(formData, institutionid);
      console.log(response);
      if (response?.status === 200) {
        toast.success("تم تعديل المؤسسة بنجاح");
        navigate(`/green_syria/dashboard/institutions/${institutionid}`);
      }
    } catch (error) {
      console.log(error);
    ...
    >
      <div className="w-full z-10 md:w-[40%] md:m-10 flex flex-col justify-center items-center">
        ...
        <p className="text-right z-10 fontBold w-[80%] md:w-[90%] my-7 text-white text-sm md:text-lg">
          تظهر لك تفاصيل المؤسسة يمكنك تعديل المؤسسة بإعادة ملأ الحقول المراد
          تعديلها
        </p>
      </div>
...
export default EditAdminAssistent;
