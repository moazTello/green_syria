...
const DetailsPlantStoreWaiting = () => {
  ...
  useEffect(() => {
    const fetch = async () => {
      const response = await fetchElement(waitingaccount, "plan");
      if (!response) {
        return;
      }
      ...
      setValue("closeTime", moment(response.closeTime).format("HH:mm:ss.SSS"));
      setValue("openTime", moment(response.openTime).format("HH:mm:ss.SSS"));
      ...
      setValue("usedId", response.plan_id);
    };
    fetch();
  }, [fetchElement, waitingaccount, setValue]);
  const approve = async () => {
    // eslint-disable-next-line no-restricted-globals
    const result = confirm("هل أنت متأكد من قبول المشتل ؟");
    if (!result) return;
  ...
  const reject = async () => {
    ...
    // eslint-disable-next-line no-restricted-globals
    const result = confirm("هل أنت متأكد من رفض المشتل ؟");
    if (!result) return;
    const formData = new FormData();
    ...
    >
      <div className="w-full md:w-[40%] md:m-10 flex flex-col justify-center items-center">
        ...
        <p className="text-right fontBold w-[80%] md:w-[90%] my-5 text-white text-sm md:text-lg">
          تظهر لك تفاصيل المشتل الذي يريد الإنضمام يمكنك قبول الإنضمام او رفضه
          مع ذكر سبب الرفض
        </p>
        <textarea
...
export default DetailsPlantStoreWaiting;
